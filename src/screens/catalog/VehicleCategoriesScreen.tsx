...
export default function VehicleCategoriesScreen() {
  ...
  const [editingCat, setEditingCat] = useState<VehicleCategory | null>(null);
  ...
  const fetchCategories = async () => {
    try {
      ...
      showToast(err.message || "Failed to load vehicle categories", "error");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      showToast(
        `Vehicle Category ${editingCat ? "updated" : "created"} successfully`,
        "success",
      );
      setShowModal(false);
      fetchCategories();
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteVehicleCategory(deletingId);
      showToast("Vehicle category deleted", "info");
      setDeletingId(null);
      fetchCategories();
    } catch (err: any) {
  ...
  const filtered = categories.filter((c) => {
    ...
    return (
      ...
      (c.searchName && c.searchName.includes(qNorm))
    );
  });
  ...
  return (
    <div className="p-6 space-y-6 w-full">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500">
                Manage top-level vehicle classifications (Car, Bike, Truck, Bus,
                etc.)
              </p>
            </div>
          </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        {loading ? (
          <div className="py-20 flex flex-col items-center justify-center text-gray-400">
            ...
            <p className="text-sm">Loading vehicle categories...</p>
          </div>
        ) : filtered.length === 0 ? (
          <div className="py-20 text-center text-gray-400">
            ...
            <p className="text-xs text-gray-500 max-w-sm mx-auto mt-1 mb-4">
              Get started by creating your first vehicle category like Car,
              Motorcycle, Scooter, or Commercial Vehicle.
            </p>
          ...
          <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4">
            <div className="bg-white rounded-2xl w-full max-w-sm shadow-2xl border border-gray-100 p-6 text-center">
              ...
              <p className="text-xs text-gray-500 mb-5">
                Are you sure you want to delete this category? Any associated
                models or variants might lose reference.
              </p>
              <div className="flex items-center justify-center gap-3">
                <button
      ...
      {/* Vehicle Data Import Modal */}
      <VehicleImportModal
        isOpen={showImportModal}
        onClose={() => setShowImportModal(false)}
        onSuccess={() => {
          showToast("Vehicle dataset imported successfully!", "success");
          fetchCategories();
        }}
      />
    </div>

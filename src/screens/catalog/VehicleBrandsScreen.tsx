...
import { VehicleCategory, VehicleBrand } from "../../models";
import {
  getVehicleCategories,
  getPaginatedVehicleBrands,
  saveVehicleBrand,
  deleteVehicleBrand,
} from "../../services/catalogService";
import { DocumentSnapshot } from "firebase/firestore";

export default function VehicleBrandsScreen() {
  ...
  const fetchPage = async (
    ...
    try {
      ...
      const res = await getPaginatedVehicleBrands({
      ...
      showToast(err.message || "Error loading vehicle brands", "error");
    } finally {
      setLoading(false);
    }
  };
  ...
  }, [selectedCatFilter, debouncedSearch]);
  ...
  const handleOpenEdit = (brand: VehicleBrand) => {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      showToast(
        `Vehicle Brand ${editingBrand ? "updated" : "created"} successfully`,
        "success",
      );
      setShowModal(false);
      fetchPage("first", 1, null);
    } catch (err: any) {
      showToast(err.message || "Error saving vehicle brand", "error");
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteVehicleBrand(deletingId);
      showToast("Vehicle brand deleted", "info");
      setDeletingId(null);
      fetchPage("first", 1, null);
    } catch (err: any) {
      showToast(err.message || "Failed to delete vehicle brand", "error");
    }
  };
    ...
    <div className="p-6 space-y-6 w-full">
        ...
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500">
              Vehicle manufacturers (e.g. Maruti Suzuki, Hyundai, Honda, Toyota,
              Tata)
            </p>
          </div>
        </div>
      ...
      <div className="bg-white p-4 rounded-2xl border border-gray-100 shadow-sm flex flex-col md:flex-row items-center justify-between gap-4">
        <div className="relative flex-1 w-full md:max-w-md">
          ...
          <input
            type="text"
            placeholder="Search directly from database by brand name..."
            value={search}
        ...
        <div className="flex items-center gap-3 w-full md:w-auto">
          ...
          <select
            ...
            className="px-3 py-2 text-xs font-medium bg-gray-50 rounded-xl border border-gray-200 focus:outline-none focus:border-orange-500 cursor-pointer"
          >
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        {loading ? (
          <div className="py-20 flex flex-col items-center justify-center text-gray-400">
            ...
            <p className="text-sm">Loading vehicle brands...</p>
          </div>
        ) : brands.length === 0 ? (
          <div className="py-20 text-center text-gray-400">
            ...
            <p className="text-xs text-gray-500 max-w-sm mx-auto mt-1 mb-4">
              Add vehicle manufacturers like Maruti Suzuki, Hyundai, Honda,
              Tata, Mahindra etc.
            </p>
            <button
              onClick={handleOpenAdd}
                    ...
                    <td className="py-3.5 px-4 text-xs font-medium text-gray-600">
                      <span className="px-2.5 py-1 bg-gray-100 rounded-lg text-gray-700">
                        {b.vehicleCategoryName ||
                          categories.find((c) => c.id === b.vehicleCategoryId)
                            ?.name ||
                          "Unknown"}
                      </span>
                    </td>
            ...
            <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl border border-gray-100 overflow-hidden animate-scale-in">
              <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
                <h2 className="text-base font-bold text-gray-900">
                  {editingBrand ? "Edit Vehicle Brand" : "Add Vehicle Brand"}
                </h2>
                <button
                ...
                <div>
                  ...
                  {categories.length === 0 && (
                    <p className="text-[11px] text-amber-600 mt-1">
                      No categories found. Please create a vehicle category
                      first!
                    </p>
                  )}
                </div>
          ...
          <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4">
            <div className="bg-white rounded-2xl w-full max-w-sm shadow-2xl border border-gray-100 p-6 text-center">
              ...
              <p className="text-xs text-gray-500 mb-5">
                Are you sure you want to delete this brand? Models attached to
                this brand may lose reference.
              </p>
